// lib/screens/driver/support_screen.dart
...
class _DriverSupportScreenState extends State<DriverSupportScreen> {
  ...
  Future<void> _openTicketDetail(Map<String, dynamic> t) async {
    await Navigator.push(
      context,
      MaterialPageRoute(
          builder: (_) => DriverSupportTicketDetailScreen(ticket: t)),
    );
    if (!mounted) return;
    await _loadTickets();
  }
...
class DriverSupportTicketDetailScreen extends StatefulWidget {
  ...
  const DriverSupportTicketDetailScreen({super.key, required this.ticket});
...
class _DriverSupportTicketDetailScreenState
    extends State<DriverSupportTicketDetailScreen> with WidgetsBindingObserver {
  static final String _BASE = AuthService.baseUrl;
